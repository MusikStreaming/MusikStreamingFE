...
export function redirectToLogin(returnUrl?: string) {
    ...
    const fullReturnUrl = returnUrl ? `${window.location.hostname}${returnUrl}` : window.location.hostname;
...
export async function signInWithGoogle(returnUrl?: string) {
    ...
    try {
        const callbackUrl = returnUrl ? `?returnUrl=${encodeURIComponent(returnUrl)}` : '/';
        ...
        throw new Error('Unable to initiate Google sign in. Please try again later.');
    }
}
